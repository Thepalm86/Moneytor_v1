...
function KPICard({ title, value, subtitle, trend, icon, color, progress, className }: KPICardProps) {
  ...
  return (
    <Card className={cn('glass-card border-premium interactive-card hover:scale-[1.02] transition-all duration-200', colors.card, className)}>
...
export function FinancialKPIDashboard({ userId, dateRange, className }: FinancialKPIDashboardProps) {
    ...
    return (
      <div className={cn('space-y-6', className)}>
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">Key performance indicators for your financial wellness</p>
        </div>
  ...
  const healthScoreColor = kpis.financialHealthScore >= 80 ? 'green' : 
                          kpis.financialHealthScore >= 60 ? 'amber' : 'red'

  const savingsRateColor = kpis.savingsRate >= 20 ? 'green' :
                          kpis.savingsRate >= 10 ? 'amber' : 'red'
    ...
    <div className={cn('space-y-6', className)}>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <KPICard
          ...
          color={kpis.emergencyFundRatio >= 1 ? 'green' : kpis.emergencyFundRatio >= 0.5 ? 'amber' : 'red'}
          progress={{
