...
import { X, Star, Sparkles, Trophy, Target, TrendingUp, Award } from 'lucide-react'
import { Dialog, DialogContent, DialogOverlay } from '@/components/ui/dialog'
import { cn } from '@/lib/utils'
import {
  playCelebrationFeedback,
  initializeCelebrationPreferences,
} from '@/lib/utils/celebration-sounds'
...
export function CelebrationModal({ isOpen, onClose, config }: CelebrationModalProps) {
  ...
  const celebrationStyles = {
    subtle: {
      backdrop:
        'bg-gradient-to-br from-slate-900/40 via-gray-900/30 to-slate-800/40 backdrop-blur-2xl',
      container:
        'relative overflow-hidden rounded-3xl bg-gradient-to-br from-white/95 via-gray-50/90 to-white/85 backdrop-blur-3xl border border-white/40 shadow-[0_32px_64px_-12px_rgba(0,0,0,0.25)] ring-1 ring-white/20',
      title:
    ...
    medium: {
      backdrop:
        'bg-gradient-to-br from-blue-900/50 via-indigo-900/40 to-purple-900/30 backdrop-blur-2xl',
      container:
        'relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-50/95 via-indigo-50/90 to-white/85 backdrop-blur-3xl border border-blue-200/60 shadow-[0_32px_64px_-12px_rgba(59,130,246,0.3)] ring-1 ring-blue-300/30',
      title:
        'text-2xl font-bold bg-gradient-to-r from-blue-800 via-indigo-700 to-purple-600 bg-clip-text text-transparent',
    ...
    major: {
      backdrop:
        'bg-gradient-to-br from-purple-900/60 via-pink-900/50 to-rose-900/40 backdrop-blur-2xl',
      container:
        'relative overflow-hidden rounded-3xl bg-gradient-to-br from-purple-50/95 via-pink-50/90 to-rose-50/85 backdrop-blur-3xl border border-purple-300/60 shadow-[0_32px_64px_-12px_rgba(168,85,247,0.4)] ring-1 ring-purple-300/40',
      title:
        'text-3xl font-bold bg-gradient-to-r from-purple-800 via-pink-700 to-rose-600 bg-clip-text text-transparent',
    ...
    epic: {
      backdrop:
        'bg-gradient-to-br from-amber-900/70 via-orange-900/60 to-red-900/50 backdrop-blur-2xl',
      container:
        'relative overflow-hidden rounded-3xl bg-gradient-to-br from-amber-50/95 via-orange-50/90 to-red-50/85 backdrop-blur-3xl border-2 border-amber-300/70 shadow-[0_32px_64px_-12px_rgba(245,158,11,0.5)] ring-1 ring-amber-400/50',
      title:
        'text-4xl font-bold bg-gradient-to-r from-amber-800 via-orange-700 to-red-600 bg-clip-text text-transparent drop-shadow-sm',
  ...
  const style =
    celebrationStyles[config.type as keyof typeof celebrationStyles] || celebrationStyles.subtle
  ...
  const animation =
    containerVariants[style.animation as keyof typeof containerVariants] || containerVariants.gentle
      ...
      {isOpen && (
        <Dialog open={isOpen} onOpenChange={() => onClose()}>
          ...
          <DialogContent className="border-0 bg-transparent p-0 shadow-none">
            <motion.div
              className={cn('relative mx-auto max-w-lg overflow-hidden', style.container)}
              variants={animation}
              initial="initial"
              animate="animate"
              exit="exit"
            >
              ...
              {/* Close button */}
              <motion.button
                className="absolute right-4 top-4 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-white/20 backdrop-blur-sm transition-all hover:scale-110 hover:bg-white/30"
                onClick={onClose}
              ...
              <div className="relative z-10 px-8 py-12 text-center">
                ...
                <motion.button
                  onClick={onClose}
                  className={cn(
                    'hover:shadow-3xl group relative overflow-hidden rounded-2xl px-8 py-4 font-bold text-white shadow-2xl transition-all duration-300 hover:scale-105 active:scale-95',
                    style.iconBg
                  )}
...
function Premium3DIcon({ type }: Premium3DIconProps) {
  const iconMap = {
    none: <Target className="h-12 w-12 text-white drop-shadow-2xl" />,
...
function _SparkleEffect({ position }: SparkleEffectProps) {
...
export function CelebrationToast({ config, onDismiss }: CelebrationToastProps) {
  ...
  const toastStyles = {
    subtle: {
      container:
        'relative overflow-hidden bg-gradient-to-r from-white/95 via-gray-50/90 to-white/85 backdrop-blur-2xl border border-gray-200/50 shadow-[0_20px_40px_-12px_rgba(0,0,0,0.15)] ring-1 ring-white/20',
      iconBg: 'bg-gradient-to-br from-slate-500 to-slate-600',
    ...
    medium: {
      container:
        'relative overflow-hidden bg-gradient-to-r from-blue-50/95 via-indigo-50/90 to-white/85 backdrop-blur-2xl border border-blue-200/50 shadow-[0_20px_40px_-12px_rgba(59,130,246,0.25)] ring-1 ring-blue-200/30',
      iconBg: 'bg-gradient-to-br from-blue-500 to-indigo-600',
    ...
    major: {
      container:
        'relative overflow-hidden bg-gradient-to-r from-purple-50/95 via-pink-50/90 to-rose-50/85 backdrop-blur-2xl border border-purple-200/50 shadow-[0_20px_40px_-12px_rgba(168,85,247,0.3)] ring-1 ring-purple-200/40',
      iconBg: 'bg-gradient-to-br from-purple-500 to-pink-600',
    ...
    epic: {
      container:
        'relative overflow-hidden bg-gradient-to-r from-amber-50/95 via-orange-50/90 to-red-50/85 backdrop-blur-2xl border border-amber-200/60 shadow-[0_20px_40px_-12px_rgba(245,158,11,0.4)] ring-1 ring-amber-300/50',
      iconBg: 'bg-gradient-to-br from-amber-500 to-orange-600',
  ...
  return (
    <motion.div
      className={cn('mx-4 flex max-w-lg items-center space-x-5 rounded-2xl p-6', style.container)}
    ...
    >
      ...
      <motion.div
        className={cn('absolute inset-0 rounded-2xl bg-gradient-to-r opacity-20', style.glowColor)}
        animate={{
          opacity: [0.1, 0.3, 0.1],
        }}
        transition={{
      ...
      <div className="relative z-10 min-w-0 flex-1">
        <motion.h4
          className={cn('mb-1.5 text-lg leading-tight', style.title)}
      ...
      <motion.button
        className="relative z-10 flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-white/20 backdrop-blur-sm transition-all hover:scale-110 hover:bg-white/30"
        onClick={onDismiss}
        whileHover={{ scale: 1.1, rotate: 90 }}
        whileTap={{ scale: 0.9 }}
...
export function CelebrationManager({ celebrations, onDismiss }: CelebrationManagerProps) {
  ...
  useEffect(() => {
    celebrations.forEach((celebration, _index) => {
  ...
  const closeModal = () => {
    ...
    const modalIndex = celebrations.findIndex(c => c.type === 'major' || c.type === 'epic')
    if (modalIndex !== -1) {
  ...
  return (
    <>
      {/* Modal celebrations */}
      {activeModal && (
        <CelebrationModal isOpen={!!activeModal} onClose={closeModal} config={activeModal} />
      )}
          ...
          {toasts.map((toast, index) => (
            <div key={index} className="pointer-events-auto">
              ...
              <div className="relative z-10">
                <CelebrationToast config={toast} onDismiss={() => dismissToast(toast)} />
              </div>
            </div>
          ))}
        </AnimatePresence>
