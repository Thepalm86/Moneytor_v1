              ...
              >
                <div className="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2">
                  <div>
                    ...
                    <div className="mt-1 font-mono text-green-600">1,234.56₪</div>
                  </div>
                  <div>
                    <span className="text-gray-600">Negative amounts:</span>
                    <div className="mt-1 font-mono text-red-600">-1,234.56₪</div>
                  </div>
                </div>
              </SettingsCard>
            )}
          ...
          <div className="flex flex-col space-y-4 sm:flex-row sm:items-end sm:space-x-4 sm:space-y-0">
            <div className="flex-1">
              ...
              >
                <SelectTrigger className="mt-2">
                  <SelectValue
                    placeholder={isTimezonesLoading ? 'Loading...' : 'Select timezone'}
                  />
                </SelectTrigger>
                <SelectContent className="max-h-60">
                  {timezoneOptions?.map(option => (
                    <SelectItem key={option.value} value={option.value}>
                      <div className="flex w-full items-center justify-between">
                        <span>{option.label}</span>
                        <span className="ml-2 text-xs text-gray-500">{option.offset}</span>
                      </div>
                    </SelectItem>
                  ))}
                </SelectContent>
        ...
        >
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            {dateFormatOptions.map(option => (
              <button
                key={option.value}
                onClick={() => setSelectedDateFormat(option.value as UserSettings['dateFormat'])}
                className={cn(
              ...
              >
                ...
                <div className="mt-1 text-sm text-gray-600">{option.example}</div>
              </button>
            ))}
          </div>
        </SettingsFormGroup>
        ...
        >
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div className="rounded-lg bg-green-50 p-4">
              <div className="mb-1 text-sm text-gray-600">Income</div>
              <div className="font-mono text-lg text-green-600">
                {formatCurrency(2500, selectedCurrency)}
              </div>
            </div>
            <div className="rounded-lg bg-red-50 p-4">
              <div className="mb-1 text-sm text-gray-600">Expense</div>
        ...
        >
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
            {CURRENCIES.slice(0, 8).map(curr => (
              <div
                key={curr.code}
                className="flex items-center space-x-2 rounded-lg bg-gray-50 p-2 text-sm"
              >
