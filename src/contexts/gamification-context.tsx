...
function gamificationReducer(state: GamificationState, action: GamificationAction): GamificationState {
  switch (action.type) {
    ...
    case 'REMOVE_CELEBRATION':
      return {
        ...state,
        pendingCelebrations: state.pendingCelebrations.filter((_, index) => index !== action.payload)
      }
...
export function GamificationProvider({ children }: GamificationProviderProps) {
  ...
  const handleTransactionLogged = useCallback(async () => {
    ...
    await checkAchievements(['first_steps', 'getting_started', 'consistent_tracker', 'dedicated_user'])
  }, [user?.id])
  ...
  const checkAchievements = useCallback(async (achievementIds: AchievementId[]) => {
    if (!user?.id) return
  ...
  const showCelebration = useCallback((config: CelebrationConfig) => {
  ...
  const getCelebrationType = (rarity: string): CelebrationConfig['type'] => {
    switch (rarity) {
      ...
      case 'epic': return 'major'
      case 'rare': return 'medium'
      case 'uncommon': return 'subtle'
...
export function useStreaks() {
  ...
  const currentStreaks: Record<StatType, number> = {}
  const bestStreaks: Record<StatType, number> = {}
